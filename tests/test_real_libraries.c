...
static void asimov(plugin_t *p, const char *dir_path) {
    ...
    for (int k = 0; k < count; ++k) {
        ...
        if (k == 0) {
            ...
            /* Its Reverb (a 4.6 s room at 50%) and Delay (0.7 s echoes, feedback 0.5,
             * at 25%) would still be ringing from the last note: take them out for
             * these two checks. At 0 each is inaudible at once. */
            p->api->set_param(p->instance, "ctl_1", "0");
            p->api->set_param(p->instance, "ctl_2", "0");
